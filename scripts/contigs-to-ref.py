...
xmap=directory + "/" + file_exten_generic + ".xmap"
...
if "-" in file_exten_generic:
    ...
    r_cmap=directory + "/" + file_exten_generic + "_r.cmap" 
    q_cmap=directory + "/" + file_exten_generic + "_q.cmap"
...
for gene, nicks in gene_nicks.items():
    ...
    for query_id, positions in query_pos_dict.items():
        ...
        # the query nick numbers used:
        query_nick1_num = nick_dict.get(query_id)[0]
        query_nick2_num = nick_dict.get(query_id)[1]

        to_print_one_file = [sample,gene,query_id,confidence,diff_bw_nick_nums.get(query_id),align_qual_flags,str(num_left_nicks),str(num_right_nicks),str(dist),nicks_used,query_nick1_num,query_nick2_num]
