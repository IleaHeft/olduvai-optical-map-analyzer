...
file_exten_generic = sys.argv[5]

# input file contig-to-ref
contig_to_ref = sys.argv[6]
...
#            ref_nick2_num = ref_CON2L1_num
    ...
    # set the reference nick numbers

for line in open(contig_to_ref):
    fields = line.strip().split("\t")
    sample = fields[0]
    gene = fields[1]
    contig = fields[2]
    ref_nick1_num=int(fields[10])
    ref_nick2_num=int(fields[11])

    
    xmap=directory + "/" + sample + "/" + file_exten_generic + contig + ".xmap"

    if "-" in file_exten_generic:
        r_cmap=directory + "/" + sample + "/" + file_exten_generic + contig + "-r.cmap" 
        q_cmap=directory + "/" + sample + "/" + file_exten_generic + contig + "-q.cmap"
    else:
        r_cmap= directory + "/" + sample + "/" + file_exten_generic + contig + "_r.cmap" 
        q_cmap= directory + "/" + sample + "/" + file_exten_generic + contig + "_q.cmap"
