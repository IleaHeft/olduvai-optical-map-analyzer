...
xmap=directory + "/" + sample + "-duf1220-mols-v-chr1.xmap"
r_cmap=directory + "/" + sample + "-duf1220-mols-v-chr1-r.cmap" 
q_cmap=directory + "/" + sample + "-duf1220-mols-v-chr1-q.cmap"
...
ref_nick_dict = defaultdict(list)
...
for gene, nicks in gene_nicks.items():
    ...
    ref_nick_dict[gene].append(ref_nick1_num)
    ref_nick_dict[gene].append(ref_nick2_num)
...
for gene, ref_nicks in ref_nick_dict.items():
    ref_nick1_num = ref_nicks[1]
    ref_nick2_num = 0 
    ...
    for line in open(xmap):
        if line.startswith("#"):
            ...
            query_gene = query_id + "_" + gene

            mol_conf[query_gene].append(confidence)
            
            if confidence >= 8:
            
            
            # check for multiple alingments with very similar confidence scores
                ...
                for align_pair in alignments:
                    ...
                    # Generate lists of all reference nick and query nick numbers in the alingment string
                    ref_nicks_in_align_str.append(ref_nick_num)
                    query_nicks_in_align_str.append(query_nick_num)

                    
                    # Check each reference nick number in the alignment string to see if it patches the DUF reference nick 1 or DUF reference nick2, if so, add molecule to list of relevant molecues
...
#                    for genes,nicks in ref_nick_dict.items():
#                        if int(ref_nick_num) in nicks:
#                            genes_per_mol_align.append(gene)
#            
#            if query_id in relevant_molecules:
#                print query_id,"\t",genes_per_mol_align

#for query_gene, item in test.items():
#    query = query_gene.split("_")[0]
#    gene = query_gene.split("_")[1]
#
#    print query,"\t",gene,"\t",item

relevant_molecules_nodups = set(relevant_molecules)
for query in relevant_molecules_nodups:
    query_gene = query + "_" +gene
    conf = mol_conf.get(query_gene)
    conf = sorted(conf)
    #pdb.set_trace()
    if len(conf) > 1:
